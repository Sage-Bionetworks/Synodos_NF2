...
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Format(digits = 2) * Heading() * (factor(Drug)) * Heading() * identity * (`IG50 (nM)` +  `Max Response (\\%)`), data = foo))
```
#Supplemental Data
...
SYN_doseResp2$IC50_nM <- replace(SYN_doseResp2$IC50_nM, SYN_doseResp2$IC50_nM>5000, ">5000")
