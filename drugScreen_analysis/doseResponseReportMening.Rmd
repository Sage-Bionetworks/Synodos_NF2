...
drugs2 <- c("AR42","Axitinib","AZD2014","Bortezomib","Everolimus","Ganetespib","GDC0941","GDC0980","Vorinostat")
...
SYN_doseResp$IC50_nM <- signif(SYN_doseResp$IC50*1000000000, 3)
SYN_doseResp$maxEfficacy100 <- signif(SYN_doseResp$maxEfficacy*100, 3)
...
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Heading() * (factor(Drug)) * Heading() * identity * Format(digits = 2) * (`IG50 (nM)` +  `Max Response (%)`), data = foo))
```
##Supplemental Response table 

```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis"}

SYN_normViab2 <- MGH_normViab[MGH_normViab$cellLine %in% c("Syn1", "Syn2", "Syn3", "Syn4", "Syn5", "Syn6", "Syn7", "Syn10", "Syn12") & MGH_normViab$drug %in% drugs2,]

SYN_doseResp2 <- doseResp[doseResp$cellLine %in% c("Syn1", "Syn2", "Syn3", "Syn4", "Syn5", "Syn6", "Syn7", "Syn10", "Syn12") & doseResp$drug %in% drugs2,]
SYN_doseResp2 <- SYN_doseResp2[order(SYN_doseResp2$IC50),]


SYN_doseResp2$IC50_nM <- round(signif(SYN_doseResp2$IC50*1000000000, 3), 1)
SYN_doseResp2$maxEfficacy100 <- round(signif(SYN_doseResp2$maxEfficacy*100, 3), 1)
...
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Heading() * (factor(Drug)) * Heading() * identity * (`IG50 (nM)` +  `Max Response (%)`), data = foo %>% filter(Drug %in% c("AR42","Axitinib","AZD2014"))))
```
##
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Heading() * (factor(Drug)) * Heading() * identity * (`IG50 (nM)` +  `Max Response (%)`), data = foo %>% filter(Drug %in% c("Bortezomib","Everolimus","Ganetespib"))))
```
##
```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis"}
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Heading() * (factor(Drug)) * Heading() * identity * (`IG50 (nM)` +  `Max Response (%)`), data = foo %>% filter(Drug %in% c("GDC0941", "GDC0980", "Vorinostat"))))
```
...
p5 <- p5 + facet_wrap(~ drug, ncol = 5) + theme_bw(base_size = 14) + theme(legend.position=c(0.9,0.2))
