...
p2
ggsave(filename = "Schwannoma_drug_response_2Dplot_Human_confidence.pdf",width = 8, height = 4)
...
foo<-distinct(filter(dplyr::select(HS_doseResp, drug, cellLine, IC50_nM, maxEfficacy100), drug %in% c("CUDC907", "Panobinostat", "GSK2126458") & cellLine %in% c("HS01", "HS11")))
...
foo2<-distinct(filter(dplyr::select(MS_doseResp, drug, cellLine, IC50_nM, maxEfficacy100), drug %in% c("CUDC907", "Panobinostat", "GSK2126458") & cellLine %in% c("MS01",  "MS02",  "MS03", "MS12",  "MSC11")))
colnames(foo2)<-c("Drug", "Cell Line", "IG50 (nM)", "Max Response (\\%)")
...
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Heading() * (factor(Drug)) * Heading() * identity * (`IG50 (nM)` + `Max Response (\\%)`), data = foo %>% filter(Drug %in% c("CUDC907", "Panobinostat", "GSK2126458"))))
