...
p1 <- p1 + guides(color=guide_legend(title="Cell Line"))
p1 <- p1 + colScaleHuman
p1
...
p2 <- p2 + guides(color=guide_legend(title="Cell Line"))
p2 <- p2 + colScaleHuman
p2
...
```{r pander, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, results="asis", fig.align="center"}
...
p3 <- p3 + guides(color=guide_legend(title="Cell Line"))
p3 <- p3 + colScaleMouse
p3
...
p4 <- p4 + guides(color=guide_legend(title="Cell Line"))
p4 <- p4 + colScaleMouse
p4
ggsave(filename = "MS_rep_drug_response_2Dplot.pdf",width = 8, height = 5)

```

##Mouse response table 

```{r pander2, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.align="center"}
MS_doseResp$IC50_nM <- MS_doseResp$IC50*1000000000

foo<-distinct(filter(select(MS_doseResp, drug, cellLine, IC50_nM, Simpson), drug %in% c("CUDC907", "GSK2126458", "Panobinostat") & cellLine %in% c("MS01", "MS02", "MS03", "MS11", "MS12")))
colnames(foo)<-c("Drug", "Cell Line", "IC50 (nM)", "Simpson AUC")
pander(foo)
```
