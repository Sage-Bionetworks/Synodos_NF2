...
group <- as.data.frame(cbind(c(rep("WT", 3),rep("Null",4)), c("HS11","MSC11", "MS12", "HS01", "MS01", "MS02", "MS03"), c("Human", rep("Mouse", 2), "Human", rep("Mouse", 3))))
...
p1 <- p1 + geom_line(data = HS_doseResp %>% filter(Species == "Human" & drug %in% drugs), aes(x = fittedX+6, y = fittedY*100, colour = Cell.Type, group = cellLine))
...
p1
ggsave(filename = "Schwannoma_drug_response_2Dplot_Human.pdf",width = 8, height = 4)
```
```{r}
...
HS_normViab <- UCF_normViab[UCF_normViab$cellLine %in% c("HS01","HS11", "MS01", "MS02", "MS03", "MS12", "MSC11") & UCF_normViab$drug %in% drugs,]

HS_doseResp <- doseResp[doseResp$cellLine %in% c("HS01","HS11", "MS01", "MS02", "MS03", "MS12", "MSC11") & doseResp$drug %in% drugs,]
HS_doseResp <- HS_doseResp[order(HS_doseResp$IC50),]
...
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Format(digits = 2) * Heading() * (factor(Drug)) * Heading() * mean * (`IC50 (nM)` +  `Max Response (%)`), data = foo))
...
colnames(foo3)<-c("Drug", "Cell Line", "IC50 (nM)", "Max Response (%)")
...
colnames(foo4)<-c("Drug", "Cell Line", "IC50 (nM)", "Max Response (%)")

foo<-signif(round(bind_rows(foo3, foo4), 1), 3)

foo$`Cell Line`[foo$`Cell Line`=="MSC11"] <- c("MS11")
foo$`Cell Line` <- factor(foo$`Cell Line`, levels = c("HS01", "HS11", "MS01",  "MS02",  "MS03","MS11", "MS12"))

latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Heading() * (factor(Drug)) * Heading() * mean * (`IC50 (nM)` +  `Max Response (%)`), data = foo %>% filter(Drug %in% c("AR42", "Axitinib", "AZD2014"))))

```
##
```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis", fig.align="center"}
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Heading() * (factor(Drug)) * Heading() * mean * (`IC50 (nM)` +  `Max Response (%)`), data = foo %>% filter(Drug %in% c("Bortezomib", "Everolimus", "Ganetespib"))))
```
##
```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis", fig.align="center"}
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Heading() * (factor(Drug)) * Heading() * mean * (`IC50 (nM)` +  `Max Response (%)`), data = foo %>% filter(Drug %in% c("GDC0941", "GDC0980", "Lapatinib"))))
```
##
```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis", fig.align="center"}
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Heading() * (factor(Drug)) * Heading() * mean * (`IC50 (nM)` +  `Max Response (%)`), data = foo %>% filter(Drug %in% c("LY2157299", "OSU0312", "Perifosine"))))
```
##
```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis", fig.align="center"}
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~  Heading() * (factor(Drug)) * Heading() * mean * (`IC50 (nM)` +  `Max Response (%)`), data = foo %>% filter(Drug %in% c("Selumetinib","Trametinib", "Vismodegib"))))
```
##
```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis", fig.align="center"}
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Heading() * (factor(Drug)) * Heading() * mean * (`IC50 (nM)` +  `Max Response (%)`), data = foo %>% filter(Drug %in% c("Vorinostat"))))
```
## All drugs (human)
...
p5 <- p5 + geom_point(aes(color=Species),size=0.1) 
p5 <- p5 + geom_line(data = comp_1_drugResp, aes(x = fittedX+6, y = fittedY*100, colour = Species, group = cellLine))
...
p6 <- p6 + geom_point(aes(color=Species),size=0.1) 
#p6 <- p6 + scale_color_brewer(type = "qual", palette = 2, direction = 1)
p6 <- p6 + geom_line(data = comp_2_drugResp, aes(x = fittedX+6, y = fittedY*100, colour = Species, group = cellLine))
...
```
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align="center", fig.width=8, fig.height=4}
HS_normViab <- UCF_normViab[UCF_normViab$cellLine %in% c("MS01", "MS02", "MS03", "MS12", "MSC11") & UCF_normViab$drug %in% drugs,]

HS_doseResp <- doseResp[doseResp$cellLine %in% c( "MS01", "MS02", "MS03", "MS12", "MSC11") & doseResp$drug %in% drugs,]
HS_doseResp <- HS_doseResp[order(HS_doseResp$IC50),]
...
p1 <- p1 + colScale
p1
ggsave(filename = "Schwannoma_drug_response_2Dplot_Mouse.pdf",width = 8, height = 4)
```