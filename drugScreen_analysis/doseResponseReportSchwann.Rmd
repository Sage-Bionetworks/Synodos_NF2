...
p1 <- p1 + guides(color=guide_legend(title="NF2 Genotype"))
p1 <- p1 + colScale
...
##Primary response table (values for MS01, MS02, MSC11 are averaged, max response based on 10 micromolar max [drug])

```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis", fig.align="center"}
HS_doseResp$IC50_nM<-round(signif(HS_doseResp$IC50*1000000000, 3),1)
HS_doseResp$maxEfficacy100 <- round(signif(HS_doseResp$maxEfficacy*100, 3),1)
...
MS_doseResp$IC50_nM<-round(signif(MS_doseResp$IC50*1000000000, 3),1)
MS_doseResp$maxEfficacy100 <- round(signif(MS_doseResp$maxEfficacy*100, 3),1)
...
names<-colnames(foo)
foo<-with(foo,aggregate(cbind(`IC50 (nM)`,`Max Response (\\%)`),list(Drug, `Cell Line`),FUN=mean))
colnames(foo)<-names
foo$`IC50 (nM)` <- replace(foo$`IC50 (nM)`, foo$`IC50 (nM)`>5000, ">5000")

latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Heading() * (factor(Drug)) * Heading() * identity * (`IC50 (nM)` + `Max Response (\\%)`), data = foo))

```
######
#Supplementa Data
##Supplemental Response Table (values for MS01, MS02, MSC11 are averaged, max response based on 10 micromolar max [drug])
```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis", fig.align="center"}
...
colnames(foo)<-names
foo$`IC50 (nM)` <- replace(foo$`IC50 (nM)`, foo$`IC50 (nM)`>5000, ">5000")
foo$`IC50 (nM)` <- replace(foo$`IC50 (nM)`, is.na(foo$`IC50 (nM)`), "NA")

latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Format(digits = 3) * Heading() * (factor(Drug)) * Heading() * identity * (`IC50 (nM)` +  `Max Response (\\%)`), data = foo %>% filter(Drug %in% c("AR42", "Axitinib", "AZD2014"))))

```
######
```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis", fig.align="center"}
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Format(digits = 3) * Heading() * (factor(Drug)) * Heading() * identity * (`IC50 (nM)` +  `Max Response (\\%)`), data = foo %>% filter(Drug %in% c("Bortezomib", "Everolimus", "Ganetespib"))))
```
######
```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis", fig.align="center"}
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Format(digits = 3) * Heading() * (factor(Drug)) * Heading() * identity * (`IC50 (nM)` +  `Max Response (\\%)`), data = foo %>% filter(Drug %in% c("GDC0941", "GDC0980", "Lapatinib"))))
```
######
```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis", fig.align="center"}
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Format(digits = 3) * Heading() * (factor(Drug)) * Heading() * identity * (`IC50 (nM)` +  `Max Response (\\%)`), data = foo %>% filter(Drug %in% c("LY2157299", "OSU03012", "Perifosine"))))
```
######
```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis", fig.align="center"}
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~ Format(digits = 3) *  Heading() * (factor(Drug)) * Heading() * identity * (`IC50 (nM)` +  `Max Response (\\%)`), data = foo %>% filter(Drug %in% c("Selumetinib","Trametinib", "Vismodegib"))))
```
######
```{r, echo=FALSE, message=FALSE, warning=FALSE, results="asis", fig.align="center"}
latex(tabular((`Cell Line` = factor(`Cell Line`)) ~Format(digits = 3) *  Heading() * (factor(Drug)) * Heading() * identity * (`IC50 (nM)` +  `Max Response (\\%)`), data = foo %>% filter(Drug %in% c("Vorinostat"))))
```
