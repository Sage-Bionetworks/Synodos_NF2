...
p1 <- p1 + colScaleHuman
p1
ggsave(filename = "HS_drug_response_2Dplot.pdf",width = 8, height = 3)
```
...
ggsave(filename = "HS_rep_drug_response_2Dplot.pdf",width = 8, height = 3)
```

##Human Response table 

```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, results="asis", fig.align="center"}
stargazer(distinct(filter(select(HS_doseResp, drug, cellLine, IC50_nM, Simpson), drug %in% c("CUDC907", "GSK2126458", "Panobinostat") & cellLine %in% c("HS01", "HS11"))), type = "html", summary = FALSE)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, results="asis", fig.align="center"}
stargazer(distinct(filter(select(MS_doseResp, drug, cellLine, IC50_nM, Simpson), drug %in% c("CUDC907", "GSK2126458", "Panobinostat") & cellLine %in% c("MS01", "MS02", "MS03", "MS11", "MS12"))), type = "html", summary = FALSE)
```
...
p5 <- p5 + colScaleHuman
p5
ggsave(filename = "compare1_drug_response_2Dplot.pdf",width = 6, height = 15)

```

##All drugs (mouse)

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align="center", fig.height=12}
...
p6 <- p6 + colScaleMouse
p6
ggsave(filename = "compare2_drug_response_2Dplot.pdf",width = 6, height = 10)
